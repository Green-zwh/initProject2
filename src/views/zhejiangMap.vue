<template>
  <div>
    <div ref="zhejiang" class="zj"></div>
  </div>
</template>

<script>
import zjmap from "../assets/data/zjMap.json";
export default {
  data() {
    return {
      geoCoordMap: {
  ...
  methods: {
    initChart() {
        this.$echarts.registerMap("浙江", zjmap); //注册地图
        let myChart = this.$echarts.init(this.$refs.zhejiang)
        let zjMapOption = {
        tooltip: {
            trigger: 'item',
